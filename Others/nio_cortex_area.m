...
% cortex, etc.).
% 
% [ Z_ST Z_EN vol ] = nio_cortex_area( BM, SegImg, options, ratio )
% -----------------------------------------------------------------
% 
...
% Example
% -------
% [ Z_ST Z_EN ] = nio_cortex_area( BM, 'template.tif', '-v' );

function [ Z_ST Z_EN varargout ] = nio_cortex_area( BM, SegImg, options, varargin )
%% read label field
info = imfinfo(SegImg);
Ml = info(1).Height;
Nl = info(1).Width;
...
num_block = length(BM);
voxel_size = round(size(BM{1}, 1)/Ml);
...
%% resize the block mask, save image index and size of every blocks
for idx = 1 : num_block
    BM{idx} = imresize(BM{idx}, [Ml, Nl]);
...
vol = 0;
for ward = 1 : num_block
    vol = vol + area(ward) * (Z_EN(ward) - Z_ST(ward)) * voxel_size * voxel_size; % unit : ��m
end
if (nargout > 2)
    varargout{3} = vol;
end
