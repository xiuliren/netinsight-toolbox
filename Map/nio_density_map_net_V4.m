...
% Uses idpar_tree
function dm_ld = nio_density_map_net_V4( network, voxel_size, rcs )
% clc
% clear
% close all
% 
...
for m = tbb(1)+rcs + 1 : voxel_size : tbb(4)-rcs -1 %rcs*2+1 :rcs*2: M-rcs*2
    m;
    for n = tbb(2)+rcs + 1 :voxel_size: tbb(5)-rcs -1
        % initialize 
        len = 0;
        
        % find the inner and crossing sections           
        overlap = (abs(m-bbc(:,1))<(bb(:,4)-bb(:,1) + 1)/2+rcs+1) & (abs(n-bbc(:,2))<(bb(:,5)-bb(:,2) + 1)/2+rcs+1);
        ...
        for sk = 1 : length( in_sec_idx )
            ...
            for idx = 1 : Nn-1
                ...
                elseif ((m2>m-rcs) && (m2<m+rcs) && (n2>n-rcs) && (n2<n+rcs))
        ...
        dm_ld( floor(m/voxel_size), floor(n/voxel_size) ) = len*1000/((2*rcs+1)*(2*rcs+1)*100);
    end
end
