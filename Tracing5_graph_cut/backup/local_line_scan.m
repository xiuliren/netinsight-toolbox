...
% by jpwu, 2013/10/17
function [node_list mk_local_stk] = local_line_scan( seed, rays, local_stk, mk_local_stk, T )
...
vray = rays{ Imax };
[C2, idxmax] =  max( vray(uint16(end/2):end,4) );
...
mk_local_stk = mark_visited_voxels(  mk_local_stk, seed, node_list );

%% mark the visited voxels for line scan
function mk_local_stk = mark_visited_voxels(  mk_local_stk, seed, P_list )
% the expansion rate
ER = 1.1;
