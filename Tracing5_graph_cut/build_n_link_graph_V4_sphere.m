...
Nv = size(co_list,1);
co_list = uint32( co_list );

% initialize the coordinate map
map_co = zeros(M,N,K,'uint32');
% for nv = 1 : Nv
...
%     pi = 1 : size(c2,1);
%     rows(rcNum + pi) = map_co( sub2ind( size(map_co),...
%         map(pi,1), map(pi,2), map(pi,3) ) );
%     cols(rcNum + pi) = map_co( sub2ind( size(map_co),...
%         c2(pi,1),  c2(pi,2),  c2(pi,3) ) );
%     idx_list = local_stk( sub2ind( size(local_stk), ...
%         map(pi,1),  map(pi,2),  map(pi,3) ) ) - ...
%         local_stk( sub2ind( size(local_stk),...
%         c2(pi,1),  c2(pi,2),  c2(pi,3) ) )  + 1;
%     weights(rcNum + pi) = wn_tab( idx_list );
%     rcNum = rcNum + size(c2,1);
...
%     pi = 2 : size(c2,1)-1;
%     rows(rcNum + pi-1) = map_co( sub2ind( size(map_co),...
%         map(pi,1), map(pi,2), map(pi,3) ) );
%     cols(rcNum + pi-1) = map_co( sub2ind( size(map_co),...
%         c2(pi-1,1),  c2(pi-1,2),  c2(pi-1,3) ) );
%     idx_list = local_stk( sub2ind( size(local_stk), ...
%         map(pi,1),  map(pi,2),  map(pi,3) ) ) - ...
%         local_stk( sub2ind( size(local_stk),...
%         c2(pi-1,1),  c2(pi-1,2),  c2(pi-1,3) ) )  + 1;
%     weights(rcNum + pi-1) = wn_tab( idx_list );
%     rcNum = rcNum + size(c2,1)-2;
%     
%     rows(rcNum + pi-1) = map_co( sub2ind( size(map_co),...
%         map(pi,1), map(pi,2), map(pi,3) ) );
%     cols(rcNum + pi-1) = map_co( sub2ind( size(map_co),...
...
map_co = zeros(M,N,K,'uint32');
map_id = zeros(Nv,4,'uint32');
% for nv = 1 : Nv
%     map_co(co_list(nv,1), co_list(nv,2), co_list(nv,3)) = nv;
%     map_id(nv,:) = [co_list(nv,:), ...
%         uint32( local_stk(co_list(nv,1), co_list(nv,2), co_list(nv,3)) )];
% end
map_co( sub2ind( size(map_co), ...
    co_list(1:Nv,1), co_list(1:Nv,2), co_list(1:Nv,3))) = 1:Nv;
map_id(1:Nv,:) = [co_list(1:Nv,:), ...
        uint32( local_stk( sub2ind(size(local_stk),...
        co_list(1:Nv,1), co_list(1:Nv,2), co_list(1:Nv,3))) )];
return;
